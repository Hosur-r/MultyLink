...
function App() {
  ...
  const req = async() => {
    await axios.get(`http://31.31.199.190/girls/yami-yoki${searchParams.get('app') !== null ? "?app=" : ""}`)
    .then((res) => {
      setGirl(res.data)
        if(res.status === 200){
          window.history.pushState({}, document.title, window.location.pathname);
        }else{
          axios.get(`http://31.31.199.190/`)
        }
  })}
  ...
  const test = async() => {
    await axios.get(`http://31.31.199.190/girls/yami-yoki`)
  }
        ...
        <div className="flex flex-col mb-4">
          ...
          {girl.avatars?.map((item, idx) => (
            <SwiperSlide key={idx} virtualIndex={idx} className='relative' >
              ...
              :  <div>
                    <div className="blur" onClick={() => {handlerClick()}}>
